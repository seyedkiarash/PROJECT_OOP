...
// #include <algorithm>
// #include <iomanip>
// #include <fstream>      // برای عملیات فایل
// #include <sstream>      // برای کار با رشته‌ها
//
// #include "Eigen/Dense" // کتابخانه Eigen برای ماتریس‌ها
//
// #ifndef M_PI
...
//     }
//
//     // Method to load available schematic files from an index file
//     void loadAvailableSchematics() {
//         schematic_files.clear();
//         string index_path = schematics_directory + "/schematics_index.txt";
//
...
//             index_file.close();
//         } else {
//             // If the index file doesn't exist, try to create it.
//             // This might fail if the directory itself doesn't exist.
//             ofstream new_index(index_path);
//             if (!new_index.is_open()) {
//                 // Cannot create index, maybe directory doesn't exist.
...
//         // First, check if the name is already in the list to avoid duplicates
//         bool found = false;
//         for (const auto& fname : schematic_files) {
//             if (fname == filename) {
//                 found = true;
//                 break;
//             }
...
//         if (!found) {
//             string index_path = schematics_directory + "/schematics_index.txt";
//             ofstream index_file(index_path, ios_base::app); // Append mode
//             if (index_file.is_open()) {
//                 index_file << filename << endl;
//                 index_file.close();
//             } else {
//                  cout << "Warning: Could not update schematics index file." << endl;
//             }
//         }
//
//         cout << "SUCCESS: Current circuit saved to '" << filename << ".txt'." << endl;
...
//         if (elements_map.count(name)) throw logic_error("Error: Resistor " + name + " already exists in the circuit");
//         double resistance = stringToDouble(val_str);
//         Resistor* res = new Resistor(getNode(n1_name), getNode(n2_name), name, resistance);
//         elements_list.push_back(res);
//         elements_map[name] = res;
//     }
//
...
//         if (elements_map.count(name)) throw logic_error("Error: Capacitor " + name + " already exists in the circuit");
//         double capacitance = stringToDouble(val_str);
//         Capacitor* cap = new Capacitor(getNode(n1_name), getNode(n2_name), name, capacitance);
//         elements_list.push_back(cap);
//         elements_map[name] = cap;
//     }
//
...
//         if (elements_map.count(name)) throw logic_error("Error: Inductor " + name + " already exists in the circuit");
//         double inductance = stringToDouble(val_str);
//         Inductor* ind = new Inductor(getNode(n1_name), getNode(n2_name), name, inductance);
//         elements_list.push_back(ind);
//         elements_map[name] = ind;
//     }
//
//     void makingDiode(const string& name, const string& n1_name, const string& n2_name, const string& model) {
//         if (elements_map.count(name)) throw logic_error("Error: Diode " + name + " already exists in the circuit");
//         IdealDiode* diode = new IdealDiode(getNode(n1_name), getNode(n2_name), name, model);
//         elements_list.push_back(diode);
//         elements_map[name] = diode;
//     }
//
...
//         if (elements_map.count(name)) throw logic_error("Error: Source " + name + " already exists in the circuit");
//         double voltage = stringToDouble(val_str);
//         VoltageSource* vs = new VoltageSource(getNode(n_plus), getNode(n_minus), name, voltage);
//         elements_list.push_back(vs);
//         elements_map[name] = vs;
//     }
//
...
//         if (elements_map.count(name)) throw logic_error("Error: Source " + name + " already exists in the circuit");
//         double current = stringToDouble(val_str);
//         CurrentSource* cs = new CurrentSource(getNode(n_plus), getNode(n_minus), name, current);
//         elements_list.push_back(cs);
//         elements_map[name] = cs;
//     }
//
//     void makingVoltageSourceSin(const string& name, const string& n_plus, const string& n_minus, const string& v_off, const string& v_amp, const string& freq) {
//          if (elements_map.count(name)) throw logic_error("Error: Source " + name + " already exists in the circuit");
//         VoltageSource* vs = new VoltageSource(getNode(n_plus), getNode(n_minus), name, 0.0);
//         vs->setSinParams(stringToDouble(v_off), stringToDouble(v_amp), stringToDouble(freq));
//         elements_list.push_back(vs);
//         elements_map[name] = vs;
//     }
...
//             try {
//                 switch(type) {
//                     case 'R': controller.makingResistor(name, n1, n2, value_part); break;
//                     case 'C': controller.makingCapacity(name, n1, n2, value_part); break;
//                     case 'L': controller.makingInductor(name, n1, n2, value_part); break;
//                     case 'V': controller.makingVoltageSourceDC(name, n1, n2, value_part); break;
//                     case 'I': controller.makingCurrentSourceDC(name, n1, n2, value_part); break;
//                     case 'D': controller.makingDiode(name, n1, n2, value_part); break;
//                     case 'E': {
//                         stringstream ss(value_part);
//                         string nc1, nc2, gain;
//                         ss >> nc1 >> nc2 >> gain;
//                         controller.makingVCVS(name, n1, n2, nc1, nc2, gain);
//                         break;
//                     }
//                     case 'G': {
//                          stringstream ss(value_part);
//                         string nc1, nc2, gain;
//                         ss >> nc1 >> nc2 >> gain;
//                         controller.makingVCCS(name, n1, n2, nc1, nc2, gain);
//                         break;
//                     }
//                     case 'H': {
//                          stringstream ss(value_part);
//                         string vctrl, gain;
//                         ss >> vctrl >> gain;
//                         controller.makingCCVS(name, n1, n2, vctrl, gain);
//                         break;
//                     }
//                     case 'F': {
//                          stringstream ss(value_part);
//                         string vctrl, gain;
//                         ss >> vctrl >> gain;
//                         controller.makingCCCS(name, n1, n2, vctrl, gain);
//                         break;
//                     }
//                     default: return false;
//                 }
//             } catch (const exception& e) {
//                 cerr << "Error creating element from file line: '" << in << "'. Reason: " << e.what() << endl;
//             }
//             return true;
//         }
//         return false;
...
//         if (regex_match(in, matches, add_res_pattern)) {
//             validateParameter("Resistance", matches[4].str(), "Resistor " + matches[1].str(), false, true);
//             controller.makingResistor(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str());
//             return true;
//         }
//
//         regex add_cap_pattern(R"(^add\s+(C\S*)\s+(\S+)\s+(\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_cap_pattern)) {
//             validateParameter("Capacitance", matches[4].str(), "Capacitor " + matches[1].str(), false, true);
//             controller.makingCapacity(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str());
//             return true;
//         }
//
//         regex add_ind_pattern(R"(^add\s+(L\S*)\s+(\S+)\s+(\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_ind_pattern)) {
//             validateParameter("Inductance", matches[4].str(), "Inductor " + matches[1].str(), false, true);
//             controller.makingInductor(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str());
//             return true;
//         }
//
//         regex add_diode_pattern(R"(^add\s+(D\S*)\s+(\S+)\s+(\S+)\s+(D|Z)$)");
//         if (regex_match(in, matches, add_diode_pattern)) {
//             controller.makingDiode(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str());
//             return true;
//         }
//
//         regex add_gnd_pattern(R"(^add\s+GND\s+(\S+)$)");
...
//             if (regex_match(val_full_str, sin_matches, vsin_params_pattern)) {
//                 validateParameter("Frequency", sin_matches[3].str(), "SIN source " + name, false, true);
//                 controller.makingVoltageSourceSin(name, n1, n2, sin_matches[1].str(), sin_matches[2].str(), sin_matches[3].str());
//                 return true;
//             }
//             regex vpulse_params_pattern(R"(^PULSE\s*\(\s*(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s*\)$)");
//             smatch pulse_matches;
...
//         if(regex_match(in, matches, add_i_pattern)){
//             validateParameter("Value", matches[4].str(), "CurrentSource " + matches[1].str(), true, false);
//             controller.makingCurrentSourceDC(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str());
//             return true;
//         }
//
//         regex add_vcvs_pattern(R"(^add\s+(E\S*)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_vcvs_pattern)) {
//              controller.makingVCVS(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str(), matches[5].str(), matches[6].str());
//              return true;
//         }
//         regex add_vccs_pattern(R"(^add\s+(G\S*)\s+(\S+)\s+(\S+)\s+(\S+)\s+(\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_vccs_pattern)) {
//             controller.makingVCCS(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str(), matches[5].str(), matches[6].str());
//             return true;
//         }
//         regex add_ccvs_pattern(R"(^add\s+(H\S*)\s+(\S+)\s+(\S+)\s+(V\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_ccvs_pattern)) {
//             controller.makingCCVS(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str(), matches[5].str());
//             return true;
//         }
//         regex add_cccs_pattern(R"(^add\s+(F\S*)\s+(\S+)\s+(\S+)\s+(V\S+)\s+(.+)$)");
//         if (regex_match(in, matches, add_cccs_pattern)) {
//             controller.makingCCCS(matches[1].str(), matches[2].str(), matches[3].str(), matches[4].str(), matches[5].str());
//             return true;
//         }
//
//         regex del_src_pattern(R"(^delete\s+([VIEGHF]\S+)$)");
...
//
//             // *** FIX STARTS HERE ***
//             // Temporarily exit file menu mode while processing the file
...
//             file.close();
//
//             // Re-enter file menu mode to display the menu again
...
//         }
//     } catch (const exception& e) {
//         // This will catch errors from stoul if the input is not a valid number
//         cout << "-Error: Inappropriate input" << endl;
//     }
//
//     // Show menu again as per PDF behavior
...
//
//     string getName() const { return name; }
//     // FIX 6: افزودن متد برای تغییر نام گره
...
// };
//
// // FIX 1: گسترش کلاس منابع برای پشتیبانی از انواع مختلف
...
//     double currentThroughSource;
//
//     // پارامترهای منابع سینوسی
//     double dcOffset, amplitude, frequency;
//     // پارامترهای منابع پالسی
//     double initialValue, pulsedValue, delayTime, riseTime, fallTime, onTime, period;
// public:
...
//
//     double getValue() const override { return value; }
//     void setValue(double val) { value = val; sourceType = DC; } // برای تحلیل DC
//     double getCurrent() const override { return currentThroughSource; }
//     void setCurrent(double current) override { this->currentThroughSource = current; }
// };
//
...
//
//             regex sin_params_pattern(R"(^SIN\s*\(\s*(\S+?)\s*,\s*(\S+?)\s*,\s*(\S+?)\s*\)$)");
//             smatch sin_matches;
